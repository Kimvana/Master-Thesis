...
## note: the A cation and ligand are assumed to be organic molecules, and should
## already be oriented/moved correctly. in the .xyz file, 0,0,0 should be the location
## of whatever is supposed to be in the centre of the unit cell.
## In case of the ligand, the length of the tail is assumed to point in the positive
## z direction in the xyz file.
## The B cation and X anion are assumed to be atomic ions
Aion = "FA"  ## FA or MA or some atom
Bion = "Pb"
Xion = "I"
Ligand = "PEA" ## PEA or nBA or ALA or PGA

## parameters for special cases. Both should be True by default, but can be 
## swithed off. In case of Crystal_2D, the value is used as a number: True
## functions like 1 in calculations, False like 0.
Aion_is_molecule = True ## to write the Aion in atom of molecule format
Crystal_2D = True ## whether to create a 2D layered structure (True), or 3D; without ligands (False)
...
Molecule_folder = "../Optimized molecules/"
Output_folder = "../gromacs input files/"
Molecule_handler = "_corr.xyz"

CellXsize = 0.64  ## size in nm
CellYsize = 0.64
CellZsize = 0.64
Gap = 2-CellZsize ## (A-A) distance around ligand gap)
...
print("   "+Aion+Bion+Xion+"3 with "+Ligand+" as the A cation site ligand")
...
Ligand_atomcount_total = NumUnitCells_stack * 2 * Ligand_atomcount * Crystal_2D
...
Output_text = Aion+Bion+Xion+"3 with "+Ligand+" as ligand; "+str(Layers)+" layer(s) of "+str(NumXUnitCells)+"*"+str(NumYUnitCells)+"*"+str(NumZUnitCells)+" cells"
